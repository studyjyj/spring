...
<head>
	...
	<script>
		$(document).ready(function name() {
			$('input[type="text"]').keyup(function name() {
				$('form').submit();
			});
			$('select').change(function name() {
				$('form').submit();
			});
		});
	</script>
	...
	<div id="wrap">
		...
		<table>
			...
			<c:if test="${empList.size() eq 0 }">
				<tr><th colspan="8">검색 결과에 맞는사원이 없습니다</th></tr>
			</c:if>
			<c:if test="${empList.size() !=0 }">
				<c:forEach var="emp" items="${empList }">
					<tr>
						...
						<td>${emp.mgr }</td>
						<td>
							<fmt:formatDate value="${emp.hiredate }" pattern="yy년 MM월DD일(E)"/>
						</td>
						<td>
